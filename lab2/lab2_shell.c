 ...
 * 可以编辑器搜索找到
 * 使用支持TODO高亮编辑器（如vscode装TODO
 * highlight插件）的同学可以轻松找到要添加内容的地方。
 */
...
int main() {
    ...
    int cmd_count;
    while (1) {
        /* TODO: 增加打印当前目录，格式类似"shell:/home/oslab
         * ->"，你需要改下面的printf */

        char *current_path =
            getcwd(NULL, 0); //获取当前工作目录，将指针返回到current_path中
        ...
        multi_cmd_count = split_string(cmdline, ";", multi_cmd);

        for (int i = 0; i < multi_cmd_count; i++) {
            /* 由管道操作符'|'分割的命令行各个部分，每个部分是一条命令 */
            /* 拆解命令行 */
            cmd_count = split_string(multi_cmd[i], "|", commands);
