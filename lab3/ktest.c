...
// 定义要输出的文件
#define OUTPUT_FILE "/home/oslab/lab/lab3/lab3-part2/expr_result.txt"
...
static my_page_referenced mpage_referenced = (my_page_referenced)0xffffffff8cf08f80;
// sudo cat /proc/kallsyms | grep follow_page
static my_follow_page mfollow_page = (my_follow_page)0xffffffff8ceea580;
...
static void print_mm_active_info(void)
{
    ...
    if (mm) {
        ...
        for (vma = mm->mmap; vma != NULL; vma = vma->vm_next) {
            ...
            int freq = 0;
            unsigned long vm_flags;
            for (virt_addr = vma->vm_start; virt_addr < vma->vm_end;
                 virt_addr = virt_addr + (1 << PAGE_SHIFT)) {
                page = mfollow_page(vma, virt_addr, FOLL_GET);

                if (!IS_ERR_OR_NULL(page)) {
                    freq = mpage_referenced(
                        page, 0, (struct mem_cgroup *)(page->memcg_data),
                        &vm_flags);
                }
                else {
                    continue;
...
static void print_seg_info(void)
{
    ...
    for (virt_addr=start; virt_addr<end; virt_addr=virt_addr+PAGE_SIZE) {
        ...
        page_start = virt_addr;
        page_end = page_start + PAGE_SIZE;

        if (!(page_start > end || page_end < start)) {
            if (start >= page_start) {
                addr = addr + (start - page_start);
            }
            real_size = min(page_end, end) - max(page_start, start);
            memcpy(buf, addr, real_size);
            write_to_file(buf, real_size);
        }
