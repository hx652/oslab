...
#define PACK(size, prev_alloc, alloc)                                          \
    ((size) & ~(1 << 1) | (prev_alloc << 1) & ~(1) | (alloc))
...
#define FTRP(bp)                                                               \
    ((char *)(bp) + GET_SIZE(HDRP(bp)) - DSIZE) /*only for free blk*/
#define NEXT_BLKP(bp) ((char *)(bp) + GET_SIZE(((char *)(bp)-WSIZE)))
#define PREV_BLKP(bp)                                                          \
    ((char *)(bp)-GET_SIZE(                                                    \
        ((char *)(bp)-DSIZE))) /*only when prev_block is free, which can usd*/
...
void mm_free(void *bp) {
    ...
    //       HDRP(bp), (size_t)GET_SIZE(HDRP(bp)), (size_t)GET_SIZE(FTRP(bp)));
...
static void *coalesce(void *bp) {
    ...
    */
    ...
    {
        int64_t head;
        head = PACK(size + GET_SIZE(HDRP(NEXT_BLKP(bp))), 1, 0);
    ...
    {
        int64_t pre_block_pre_alloc;
        int64_t head;
        pre_block_pre_alloc = GET_PREV_ALLOC(HDRP(PREV_BLKP(bp)));
        head =
            PACK(size + GET_SIZE(HDRP(PREV_BLKP(bp))), pre_block_pre_alloc, 0);

        PUT(FTRP(bp), head);
        PUT(HDRP(PREV_BLKP(bp)), head);
    ...
    else /*前后都是空闲块*/
    {
        ...
        int64_t head;
        pre_block_pre_alloc = GET_PREV_ALLOC(HDRP(PREV_BLKP(bp)));
        head = PACK(size + GET_SIZE(HDRP(PREV_BLKP(bp))) +
                        GET_SIZE(FTRP(NEXT_BLKP(bp))),
                    pre_block_pre_alloc, 0);
...
static void *find_fit_best(size_t asize) {
    ...
    */
    ...
    while (p != NULL) {
        if (GET_SIZE(HDRP(p)) >= asize) {
            if (MAX(min_diff, GET_SIZE(HDRP(p)) - asize) == min_diff) {
                best_p = p;
                min_diff = GET_SIZE(HDRP(p)) - asize;
            }
        }
        p = GET_SUCC(p);
    }
...
static void place(void *bp, size_t asize) {
    /*
        ...
        HINTS:
            1. 若空闲块在分离出一个 asize
       大小的使用块后，剩余空间不足空闲块的最小大小，
                则原先整个空闲块应该都分配出去
            2.
       若剩余空间仍可作为一个空闲块，则原空闲块被分割为一个已分配块+一个新的空闲块
    ...
    */
    ...
    // the rest is enough or not
    delete_from_free_list(bp);
    if ((cur_size - asize) >= MIN_BLK_SIZE) {
        int64_t head;
        head = PACK(asize, GET_PREV_ALLOC(HDRP(bp)), 1);

        PUT(HDRP(bp), head);
        bp = NEXT_BLKP(bp);

        int64_t new_head;
        new_head = PACK(cur_size - asize, 1, 0);
        PUT(HDRP(bp), new_head);
        PUT(FTRP(bp), new_head);
        add_to_free_list(bp);
    }
    else {
        int64_t head;
        head = PACK(cur_size, GET_PREV_ALLOC(HDRP(bp)), 1);

        PUT(HDRP(bp), PACK(cur_size, GET_PREV_ALLOC(HDRP(bp)), 1));
...
void mm_check(const char *function) {
    ...
    {
        count_empty_block++;
        printf("addr_start:%u, addr_end:%u, size_head:%u, size_foot:%u,PRED = "
               "%u,SUCC = %u \n ",
               (size_t)bp - WSIZE, (size_t)FTRP(bp), GET_SIZE(HDRP(bp)),
               GET_SIZE(FTRP(bp)), GET_PRED(bp), GET_SUCC(bp));
        ;
        bp = (char *)GET_SUCC(bp);
    }
