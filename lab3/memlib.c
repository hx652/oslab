...
void *mem_sbrk(int incr) {
    ...
    /*
        TODO:
            ...
            返回值: 旧 mem_brk 值
        HINTS:
        1. 若 mem_brk + incr 没有超过实际的 mem_max_addr 值，直接推进 mem_brk
       值即可
        2. 若 mem_brk + incr 超过实际的 mem_max_addr 值，需要调用 sbrk
       为内存分配器掌管的内存扩容
        3. 每次调用 sbrk 时， mem_max_addr 增量以 MAXHEAP对齐
    */
    char *new_brk = (char *)(mem_brk + incr);
    if (incr < 0) {
        return (void *)-1;
    }
    else if (new_brk <= mem_max_addr) {
        mem_brk = new_brk;
    }
    else if (new_brk > mem_max_addr) {
        sbrk(MAX_HEAP);
